...
public:
    ...
    RootsType roots() const // 问题：两个线程同时调用某个对象的该函数时，产生线程安全性问题。解决方法：引入互斥量
    {
        /*
            添加以下代码
        */
        std::lock_guard<std::mutex> g(m); // 加上互斥量 lock_guard 是一个模板类，利用类的构造和析构管理互斥量的加锁和解锁
        if (!rootsAreValid)
        {
            // 计算 并存入 rootVals
...
private:
    ...
    mutable std::mutex m; // 互斥量 std::mutex是个只移型别，即只能移动但不能复制，导致类本身也失去了可复制性，但仍可移动
};
...
private:
    mutable std::atomic<unsigned> callCount{0}; // 只移型别 对类的影响同上文 mutex
    double x, y;
};
