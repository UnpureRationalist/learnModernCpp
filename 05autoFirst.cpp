...
class Widget {
 ...
 public:
  bool operator<(const Widget &rop) const { return data < rop.data; }
};
...
int main() {
  ...
  /*
  for(pair<string, int> &p : m)   // 报错，原因：哈希表中键值类型为const
  应该声明为 pair<const string, int>
  {
