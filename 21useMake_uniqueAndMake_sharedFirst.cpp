...
/*
    ...
    对于 shared_ptr ，不建议使用 make 系列函数的额外场景包括：
        1. 自定义内存管理的类
        2. 内存紧张的系统、非常大的对象、以及存在比指涉到相同对象的 shared_ptr 生存期更久的 weak_ptr
*/
...
int main()
{
    ...
    std::unique_ptr<Widget> upw2(new Widget()); // use new, now recommend

    auto spw1(make_shared<Widget>());

    // make 系列函数能保证异常安全
    proessWidget(shared_ptr<Widget>(new Widget), computePriority()); // 导致潜在的资源泄露
    proessWidget(make_shared<Widget>(), computePriority());          // 不会发生潜在的资源泄露风险
